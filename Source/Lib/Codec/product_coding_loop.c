...
static void md_encode_block_light_pd1(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t sb_addr,
    ...
    if (pcs->ppcs->blk_lambda_tuning) {
        ...
        || pcs->ppcs->scs->static_config.tune == 4 || pcs->ppcs->scs->static_config.tune == 5) {
...
static void md_encode_block(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t sb_addr,
    ...
    ctx->tune_ssim_level = (pcs->scs->static_config.tune == 2 && ctx->pd_pass == PD_PASS_1) ? SSIM_LVL_3 : ((pcs->scs->static_config.tune == 3
        || pcs->scs->static_config.tune == 4 || pcs->ppcs->scs->static_config.tune == 5) && ctx->pd_pass == PD_PASS_1) ? SSIM_LVL_1 : SSIM_LVL_0;
    md_stage_3(pcs,
               ctx,
               input_pic,
               loc.input_origin_index,
